      ...
      <main className="bg-white">
        <section className="px-5 py-16 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <Badge className="bg-red-50 text-red-800 hover:bg-red-50">{project.category}</Badge>
            ...
            <div className="relative mt-10 aspect-[16/9] overflow-hidden rounded-lg bg-zinc-100">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {project.gallery.map((image) => (
                <div key={image} className="relative aspect-[4/3] overflow-hidden rounded-lg bg-zinc-100">
                  <Image src={image} alt={`${project.title} - detalhe`} fill sizes="(min-width: 768px) 33vw, 100vw" className="object-cover" />
                </div>
              ))}
            </div>
            <Link href="/contato" className="mt-10 inline-flex items-center gap-2 font-semibold text-red-700">
              Falar sobre um projeto semelhante <ArrowRight className="size-4" />
            </Link>
          </div>
        </section>
      </main>
